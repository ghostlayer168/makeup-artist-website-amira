...
const PageUtils = {
    ...
    getTargetUrl(targetLang, currentPageInfo) {
        ...
        // Determine filename for target language
        let targetFilename = '';
        
        if (targetLang === 'en') {
            // English files without suffix
            targetFilename = baseName.replace('_ar', '').replace('_sv', '');
        } else if (targetLang === 'sv') {
            // Swedish files with _sv suffix
        ...
        // Return path relative to current folder
        if (currentLang === targetLang) {
            // If same language, stay on same page
            return targetFilename;
        } else {
            // Go to different language folder
...
const GalleryPageLoader = {
    ...
    initPreloader() {
        ...
        const messages = [
            "Laddar Galleriresurser",
            "Initierar Bilder",
            "Förbereder Upplevelse",
            "Nästan Klart"
        ];
...
const GalleryNavigation = {
    ...
    generateMobileMenuContent() {
        const currentPageInfo = PageUtils.getCurrentPageInfo();
        const activeLang = currentPageInfo.lang.toUpperCase() || 'EN';
        
        // Generate correct links based on current language
        const enLinks = {
            home: '../en/index.html',
            philosophy: '../en/index.html#philosophy',
        ...
        const svLinks = {
            home: '../sv/index_sv.html',
            philosophy: '../sv/index_sv.html#philosophy',
            services: '../sv/services_sv.html',
            gallery: '../sv/gallery_sv.html',
            contact: '../sv/index_sv.html#contact'
        };
        
        const arLinks = {
            home: '../ar/index_ar.html',
            philosophy: '../ar/index_ar.html#philosophy',
            services: '../ar/services_ar.html',
            gallery: '../ar/gallery_ar.html',
            contact: '../ar/index_ar.html#contact'
        };
        
        // Select correct links based on current language
        let links = enLinks;
        if (activeLang === 'SV') links = svLinks;
        if (activeLang === 'AR') links = arLinks;
        ...
        return `
            <div class="mobile-menu-container">
                ...
                <nav class="mobile-menu-nav">
                    <a href="${links.home}" class="mobile-nav-link ${currentPageInfo.filename.includes('index') ? 'active' : ''}">
                        <span class="link-number">01</span>
                        <span class="link-text">Hem</span>
                    </a>
                    
                    <a href="${links.philosophy}" class="mobile-nav-link">
                        <span class="link-number">02</span>
                        <span class="link-text">Filosofi</span>
                    </a>
                    
                    <a href="${links.services}" class="mobile-nav-link ${currentPageInfo.filename.includes('services') ? 'active' : ''}">
                        <span class="link-number">03</span>
                        <span class="link-text">Tjänster</span>
                    </a>
                    
                    <a href="${links.gallery}" class="mobile-nav-link ${currentPageInfo.filename.includes('gallery') ? 'active' : ''}">
                        <span class="link-number">04</span>
                        <span class="link-text">Portfolio</span>
                    </a>
                    
                    <a href="${links.contact}" class="mobile-nav-link">
                ...
                <div class="mobile-language-section">
                    <div class="language-label">Välj Språk</div>
                ...
                <a href="${links.contact}" class="mobile-cta-btn">
    ...
    handleLanguageSelect(e) {
        ...
        // Use PageUtils for correct URL
        const currentPageInfo = PageUtils.getCurrentPageInfo();
        const targetUrl = PageUtils.getTargetUrl(lang, currentPageInfo);
        
        console.log(`Switching language to: ${langCode}, from: ${currentPageInfo.filename}, to: ${targetUrl}`);
    ...
    handleMobileNavLinkClick(e) {
        ...
        setTimeout(() => {
            if (href.includes('.html')) {
                ...
                if (href === '#hero' && window.location.pathname.includes('gallery')) {
                    window.scrollTo({
                        top: 0,
                        behavior: 'smooth'
                    });
...
const GalleryLanguage = {
    ...
    selectLanguage(e) {
        ...
        if (typeof gsap !== 'undefined') {
            gsap.to(option, {
                ...
                repeat: 1,
                onComplete: () => {
                    // Determine redirect URL using PageUtils
    ...
    redirectToLanguage(lang) {
        ...
        console.log(`Redirecting from ${currentPageInfo.filename} to: ${targetUrl}`);
...
const Gallery = {
    ...
    updateGalleryCounter(filteredCount) {
        ...
        if (filteredElement) {
            ...
            filteredElement.textContent = showingCount;
        }
    },
    ...
    updateLoadMoreButton(filteredCount) {
        ...
        if (remaining > 0) {
            ...
            loadMoreBtn.innerHTML = `
                <span>Ladda Fler (${remaining} kvar)</span>
...
const GalleryModal = {
    ...
    collectGalleryImages() {
        ...
        return Array.from(galleryItems).map((item, index) => {
            ...
            const title = item.querySelector('.image-title')?.textContent || `Galleri Bild ${index + 1}`;
            const category = item.querySelector('.image-category')?.textContent || 'Galleri';
    ...
    generateDescription(title, category) {
        const descriptions = {
            bridal: `En fantastisk bröllopstransformation med titeln "${title}". Denna look kombinerar traditionella östliga bröllopstekniker med modern elegans, med intrikata detaljer och lyxiga finish som skapar en tidlös skönhet perfekt för din speciella dag.`,
            editorial: `Redaktionellt mästerverk "${title}" som visar högmode makeupkonst. Denna look skapades för en professionell fotografering, som blandar kreativt uttryck med teknisk precision för att producera bilder som berättar en gripande visuell historia.`,
            traditional: `Traditionell makeup look "${title}" som hyllar kulturellt arv. Denna design inkorporerar autentiska östliga skönhetstekniker som gått i arv genom generationer, återuppfunnit med samtida sofistikering för moderna firanden.`,
            events: `Specialevents makeup "${title}" designad för minnesvärda tillfällen. Denna glamorösa look säkerställer att du sticker ut med självförtroende, med långvariga formler och kameraklära finish som håller timmar av firande.`,
            special: `Kreativ specialeffekts makeup "${title}" som demonstrerar konstnärlig innovation. Denna transformerande look flyttar gränserna för makeupkonst, kombinerar teknisk skicklighet med fantasifull design för att skapa verkligen unika visuella upplevelser.`
        };
        
        return descriptions[category.toLowerCase()] || 
               `En fantastisk makeupkonstarbete med titeln "${title}" från Amiras portfolio. Detta mästerverk visar den perfekta balansen mellan traditionella tekniker och modern estetik, som skapar skönhet som överstiger trender och firar individualitet.`;
    },
    ...
    updateModalContent(imageData) {
        ...
        if (totalImages) {
            totalImages.textContent = this.imagesData.length;
        }
    ...
    updateTechTags(category) {
        ...
        // Tags by category
        const tags = {
            bridal: ['Traditionell', 'Guldaccents', 'Handmålad', 'Bröllop', 'Lyx'],
            editorial: ['Redaktionell', 'Högmode', 'Kameraklädd', 'Kreativ', 'Konstnärlig'],
            traditional: ['Kulturell', 'Arv', 'Traditionell', 'Autentisk', 'Östlig'],
            events: ['Event', 'Glamorös', 'Långvarig', 'Kamerablixt-säker', 'Elegant'],
            special: ['Specialeffekter', 'Kreativ', 'Konstnärlig', 'Transformerande', 'Innovativ']
        };
        ...
        // Add tags
        const categoryTags = tags[category.toLowerCase()] || ['Makeup', 'Konst', 'Skönhet', 'Lyx'];
