...
const GalleryModal = {
    ...
    collectGalleryImages() {
        ...
        return Array.from(galleryItems).map((item, index) => {
            ...
            const title = item.querySelector('.image-title')?.textContent || `Gallery Image ${index + 1}`;
            const category = item.querySelector('.image-category')?.textContent || 'Gallery';
    ...
    generateDescription(title, category) {
        const descriptions = {
            bridal: `A breathtaking bridal transformation titled "${title}". This look combines traditional Eastern bridal techniques with modern elegance, featuring intricate detailing and luxurious finishes that create a timeless beauty perfect for your special day.`,
            editorial: `Editorial masterpiece "${title}" showcasing high-fashion makeup artistry. This look was created for a professional photoshoot, blending creative expression with technical precision to produce images that tell a compelling visual story.`,
            traditional: `Traditional makeup look "${title}" honoring cultural heritage. This design incorporates authentic Eastern beauty techniques passed down through generations, reimagined with contemporary sophistication for modern celebrations.`,
            events: `Special event makeup "${title}" designed for memorable occasions. This glamorous look ensures you stand out with confidence, featuring long-lasting formulas and camera-ready finishes that withstand hours of celebration.`,
            special: `Creative special effects makeup "${title}" demonstrating artistic innovation. This transformative look pushes the boundaries of makeup artistry, combining technical skill with imaginative design to create truly unique visual experiences.`
        };
        
        return descriptions[category.toLowerCase()] || 
               `A stunning makeup artistry work titled "${title}" from Amira's portfolio. This masterpiece showcases the perfect balance between traditional techniques and modern aesthetics, creating beauty that transcends trends and celebrates individuality.`;
    },
    ...
    updateTechTags(category) {
        ...
        // Tags by category
        const tags = {
            bridal: ['Traditional', 'Gold Accents', 'Hand-painted', 'Bridal', 'Luxury'],
            editorial: ['Editorial', 'High Fashion', 'Camera Ready', 'Creative', 'Artistic'],
            traditional: ['Cultural', 'Heritage', 'Traditional', 'Authentic', 'Eastern'],
            events: ['Events', 'Glamorous', 'Long-lasting', 'Camera Flash-proof', 'Elegant'],
            special: ['Special Effects', 'Creative', 'Artistic', 'Transformative', 'Innovative']
        };
        ...
        // Add tags
        const categoryTags = tags[category.toLowerCase()] || ['Makeup', 'Artistry', 'Beauty', 'Luxury'];
